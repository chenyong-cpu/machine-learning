...
class CNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, bn_act=True, **kawargs):
        self.conv = nn.Conv2d(in_channels, out_channels, bias=not bn_act, **kawargs)
    ...
    def forward(self, x):
        if self.use_bn_act:
            return self.leaky(self.nm(self.conv))
        else:
            return self.conv(x)
