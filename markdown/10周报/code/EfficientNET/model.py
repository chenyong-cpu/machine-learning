...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, reduced_dim):
        super(SqueezeExcitation, self).__init__()
        self.se == nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels, reduced_dim, 1),
            nn.SiLU(),
            nn.Conv2d(reduced_dim, out_channels, 1),
            nn.Sigmoid(),
        )
...
class InvertedResidualBlock(nn.Module):
    ...
    def stochastic_depth(self, x):
        ...
        binary_tensor = torch.rand(x.shape[0], 1, 1, 1, devie=x.device) < self.survival_prob
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, version, num_classes):
        ...
        width_factor, depth_factor, dropout_rate = self.calcalate_factors(version)
